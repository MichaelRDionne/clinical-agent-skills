...
| [`clinical-change-control`](skills/clinical-change-control/SKILL.md) | The 14 hard gates of a PHI-touching EHR-automation pipeline, each with its incident: mount sentinels, attested browser surfaces, payload-reality checks, identity quotes, archive-only cleanup, and a mechanical pre-merge payload screen that catches honest-but-empty refusals human review approved. |
