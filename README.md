...
| [`clinical-change-control`](skills/clinical-change-control/SKILL.md) | The 14 numbered hard gates plus 14b (array item shapes) of a PHI-touching EHR-automation pipeline, each with its incident: mount sentinels that prove the backing store is still there, attested browser surfaces, payload-reality checks, identity quotes, archive-only cleanup, and a mechanical pre-merge payload screen that catches honest-but-empty refusals and shape-wrong arrays human review approved. |
...
| [`portfolio-safety-check`](skills/portfolio-safety-check/SKILL.md) | Pre-publish audit for a professional's public GitHub: the employer/vendor/colleague/codename layer that generic secrets scanners miss, git-history leak checks on both tree content and commit messages, a claim-honesty pass (can you defend every README claim in an interview?), a stale-clone concurrency gate, SHA-pinned live verify, and the flip-private → genericize → verify-live remediation pattern. (This repo passed its own check before publishing.) |
| [`writing-without-ai-tells`](skills/writing-without-ai-tells/SKILL.md) | Why AI prose reads as AI, and what to do instead: the two underlying drivers (over-explaining, performed rigor), the systemic levers that suppress them, a scoring-based calibration procedure, an experience-injection method, and a QC appendix of named tells — including work-preamble ceremony, a four-rung validation ladder, and a keyword guard showing that the words detectors flag hardest are the ones readers never cite. |
| [`outbound-send-gate`](skills/outbound-send-gate/SKILL.md) | The operator sends; agents stage. Seven process gates, each with the incident that created it: a sent-count sentinel that covers every send-as identity, no cached coordinates near Send, clipboard verified at paste time, the saved draft (not the compose window), From confirmed from the DOM, permission-stops that stay stopped, and first-person practice claims that need a source or `[TO FILL]`. |
| [`evidence-check`](skills/evidence-check/SKILL.md) | Verify a clinical or drug claim against primary sources — openFDA / DailyMed labels and PubMed — when the local textbook library cannot confirm it. Free public APIs, no key, no PHI in queries. Includes the 420-byte FDA stub that returns HTTP 200 to a default curl User-Agent. |
| [`adhd-mode`](skills/adhd-mode/SKILL.md) | Shape output for a reader with ADHD: lead with the next action, number the steps, restate state every turn, suppress tangents, make wins visible. Auto-triggers on operational work; stays out of prose drafting and clinical reasoning. |
