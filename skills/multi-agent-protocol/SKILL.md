...
- **Live external-system sessions have no compare-and-swap and need their own peer check.** Every rule above protects markdown on a shared filesystem. A logged-in browser session against a single-session external system — a lab-ordering portal, an EHR, any web app that keeps one server-side session per org — has no equivalent protection: no edit tool, no "modified on disk," no conflict copy, and often no audit trail either. Two agents drove the same portal against the same record about twenty minutes apart, unaware of each other. What made it possible is the part worth remembering: **one agent's own checkpoint read "blocked — decision needed" on the exact action the other then executed**, and the second agent read that as an idle lane rather than a live one parked mid-task. A checkpoint saying "blocked" describes the work, never the agent. Before driving any such surface, check for a running peer process (`ps aux | grep -i <peer-tool>`, case-insensitive) and check the freshness of the relevant workstream checkpoint's own timestamp.
