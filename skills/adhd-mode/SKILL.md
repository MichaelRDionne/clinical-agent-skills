...
name: adhd-mode
description: >-
  Shape output for a reader with ADHD. Auto-trigger on operational,
  instructional, and multi-step work — setup tasks, debugging, config
  walkthroughs, anything where the operator executes steps by hand — and on any
  long multi-turn task where tracking progress matters. Output leads with the
  concrete next action, numbers multi-step work, restates state every turn,
  suppresses tangents, and makes wins visible. Do not auto-trigger for prose
  drafting, clinical or psychiatric reasoning, or research reads — those need
  full-length answers. The operator can force it on anywhere with /adhd-mode.
---
...
### 6. Give specific time estimates — and label guesses as guesses

Vague estimates fail. Ballpark in concrete units. An invented number stated
confidently is worse than a vague one — if the estimate is a guess, say so
in two words.
...
Never use "Uh oh," "Oh no," or "There seems to be a problem." State cause
and fix.
...
1. The operator asks to "explain" or "walk me through." Explain fully. Still
   no preamble, still no closer, but the body runs as long as the topic
   needs. Add headers so the reader can skim back.
...
5. Clinical or domain-expert reasoning the operator asked for in full. The
   action-first structure can stay, but never compress that reasoning to hit
   a length target.
...
4. Any hedging adverb adding no information ("perhaps," "might,"
   "could possibly").
